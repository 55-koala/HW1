...
```
## 困難與挑戰
- 資料格式不一致:JokeAPI 的資料有兩種格式：單句型 (single) 和問答型 (twopart)，需要根據 type 欄位判斷資料並分別處理。
- 資料量與速度限制:如果請求過多資料（例如一次抓100筆），可能會遇到伺服器拒絕請求，因此需要控制每次請求的資料數。
- 連線失敗風險:API 可能會因網路不穩或伺服器問題而無法回應，需使用錯誤處理機制來確保程式穩定運行。
